...
class ExperimentVisualizer:
    ...
    def get_heatmap(self, file_name: str):
        ...
        totals = self.total_data_file.groupby("index")["training_iteration"].max().values
    ...
    def plot_loss_curves(self, plot_name: str):
        ...
        plt.plot(time_range, self.simulated_loss, alpha=0.1, color="blue", label="Simulated Loss")
        plt.plot(time_range, self.true_loss, alpha=0.15, color="red", label="True Mean Schedule")
        best_path = self.total_data_file.groupby("training_iteration")["loss"].min().values
        plt.plot(time_range, best_path, label="Scheduler Chosen Path Best Value", color="cyan")
        best_average_worker_path = self.total_data_file.groupby("training_iteration")["loss"].apply(lambda x: x.nsmallest(self.num_workers).mean())
        plt.plot(time_range, best_average_worker_path, label=f"Scheduler Chosen Average {self.num_workers}-Worker Path Value", color="lime")
    ...
    def calculate_regret(self):
        ...
        for i in range(1, self.max_num_epochs):
            ...
            avg = (np.sum(values) - (len(values)/self.num_workers)*best_arm_sum)/len(values)
            avgs.append(avg)
            running_sum += avg
    ...
    def combined_seed_data_regret_curve(self, experiment_list, plot_name: str):
        ...
        for experiment in experiment_list:
            cumulative_avg = np.vstack([cumulative_avg, experiment.average_regret])
