...
class PredASHABracket(BaseBracket):
    ...
    def cutoff(
        ...
        if not index or index == (self.MAX_RUNGS - 1):
            return np.nanpercentile(list(recorded.values()), (1 - 1 / self.reduction_factor1) * 100)
        else:
            ...
            return np.nanpercentile(vals, (1 - 1 / self.reduction_factor1) * 100)
        ...
        for i, milestone, recorded in enumerate(self.rungs):
            ...
            if cur_iter < milestone or trial.trial_id in recorded:
                ...
                if i != self.MAX_RUNGS - 1:
                    ...
                    print("print prediction based: ", pred_cur_rew, cur_rew, cutoff)
...
if __name__ == "__main__":
    sched = PredAsyncHyperBandScheduler(grace_period=1, max_t=10, reduction_factor=2)
